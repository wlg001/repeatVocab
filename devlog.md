...
### Commit 43: feat: 添加Alt键显示句子提示功能 ⌨️
**用户需求：** 句子模式下，按Alt键，直接完整的显示句子拼写

**实现内容：**

#### 1. handleGlobalKeyboard方法扩展
- 判断当前练习类型（单词/句子）
- 单词模式：Alt键播放发音（原功能）
- 句子模式：Alt键显示完整英文句子
- 防止默认事件，避免浏览器行为干扰

#### 2. showSentenceHint新方法
- 显示当前句子的完整英文拼写
- 使用`correct-answer-display`容器
- 带💡图标提示
- 显示中文翻译作为参考
- 2秒后自动隐藏
...
**代码变更：**
- `app.js`:
  - `handleGlobalKeyboard()`: 添加句子模式分支判断
  - `showSentenceHint()`: 新增方法显示句子提示
...
### Commit 44: ui: 扩大句子输入框宽度适配长句子 📏
**用户需求：** 句子输入框，可能不够长，不能显示下整个句子

**优化内容：**
- 输入框最大宽度：500px → **800px**（增加60%）
- 字体大小：20px → **18px**（在更宽输入框中保持更好比例）
- 确保长句子可以完整显示

**代码变更：**
- `style.css`: `.word-input` 样式调整
...
### Commit 45: feat: 重复添加单词/句子时合并释义和标签，熟练度-10 🔄
**用户需求：** 对于重复添加的单词，合并更新新的中文释义，合并更新新的标签，熟练度-10

**实现逻辑：**

#### 单词处理
1. **合并释义**：去重合并已有释义和新增释义
2. **合并标签**：去重合并已有标签和新增标签
3. **熟练度调整**：当前值 -10（不低于-100）
4. **确认对话框**：详细显示合并前后对比
5. **统计数据保留**：practiceCount、correctCount等不变

#### 句子处理
1. **合并翻译**：支持多个可选翻译，用分号分隔
2. **合并标签**：与单词相同逻辑
3. **熟练度调整**：当前值 -10（不低于-100）
4. **确认对话框**：显示合并详情
5. **统计数据保留**：保持练习历史
...
- `app.js`:
  - `addWord()`: 修改重复单词处理逻辑
  - `bulkImport()`: 批量导入时应用相同逻辑
  - `addSentence()`: 修改重复句子处理逻辑
  - `bulkImportSentences()`: 批量导入句子时应用相同逻辑
...
#### 核心功能实现
1. ✅ **句子练习模块**
   - 完整的句子数据层（12个Storage方法）
   - SentencePracticeManager类（练习逻辑）
   - 句子管理UI（批量导入、单个添加、列表CRUD）
   - 忽略标点符号的答案验证
   - 5次错误阈值显示答案
...
3. ✅ **统计功能扩展**
   - 分开显示单词和句子统计
   - 5张统计卡片（单词数、句子数、练习次、正确数、错误数）
   - 打卡日历支持过滤（显示单词/句子）
   - 合并两个练习日志显示

4. ✅ **UI/UX优化**
   - 句子输入框扩展至800px
   - 打卡日历视觉升级（渐变色、更大字体、流畅动画）
   - Alt键快捷键支持句子提示
   - 统一的布局和样式设计
...
#### 技术架构
- **数据层**：扩展Storage类，新增句子相关方法
- **业务逻辑**：SentencePracticeManager类
- **UI控制**：UIController类扩展
- **样式系统**：保持VSCode深色主题一致性

#### 文件变更统计
- `app.js`: +800行（句子模块、混合练习、统计扩展）
- `index.html`: +150行（句子管理UI、练习类型选择）
- `style.css`: +200行（日历优化、输入框扩展）
- `devlog.md`: 完整记录10个提交（Commit 36-45）
- `README.md`: 更新功能说明、使用指南、数据结构
