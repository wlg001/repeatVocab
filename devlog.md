...
#### 1. 统计概览扩展
- 新增"总句子数"卡片，显示句子总数
- "总练习次数/正确/错误"包含单词和句子的合计数据
- 5个统计卡片清晰展示全局数据

#### 2. 打卡日历增强
- 新增"显示单词/显示句子"复选框，可独立控制
- 支持三种显示模式：
  * 仅单词：显示 "X词 Y对"
  * 仅句子：显示 "X句 Y对"
  * 单词+句子：显示 "X词 Y句 Z对"
- 合并两个练习日志（wordPracticeLog + sentencePracticeLog）
- 日历图例更新为通用描述（"1-5个"而非"1-5个单词"）
...
**代码变更：**
- `index.html`:
  - 添加 `total-sentences` 统计卡片
  - 添加日历控制复选框（calendar-show-words/sentences）
  - 添加 `sentence-stats-list` 容器
  - 日历图例文本通用化
- `app.js`:
  - `updateStats()`: 合并单词和句子的统计数据，渲染两个独立列表
  - `updateCalendar()`: 读取两个日志，根据复选框状态合并显示
  - `bindEvents()`: 绑定日历复选框的change事件
...
### Commit 42: ui: 优化打卡日历外观设计 ✨
**用户需求：** 打卡日历里的字显示的太丑了，能否显示的更好看些

**设计改进：**

#### 1. 整体布局优化
- 日历容器增大：max-width从600px → 700px
- 圆角更圆润：4px → 8px
- 内边距增加：15px → 20px
- 网格间距加大：3px → 6px，日期方块更清晰

#### 2. 字体和文字优化
- 标题字号增大：14px → 16px，字重400 → 500
- 星期标题颜色：#858585 → #9cdcfe（更明亮）
- 星期标题字重：400 → 500，增加letter-spacing
- 日期数字：11px → 14px，字重500
- 练习计数：9px → 10px，更易读

#### 3. 日期方块美化
- 最小高度：40px → 60px（更方正，信息展示更充分）
- 圆角：2px → 6px（更现代）
- 悬停效果改进：
  * scale变换 → translateY向上浮动
  * 添加阴影效果
  * 过渡时间：0.15s → 0.2s ease
- 今日标记阴影增强：更明显的发光效果

#### 4. 练习强度颜色升级
- 使用线性渐变替代纯色：`linear-gradient(135deg, ...)`
- 低强度：#1a3a1a → 渐变(#1e3a2e → #1a3a1a)
- 中强度：#2a5a2a → 渐变(#2e5a3e → #2a5a2a)
- 高强度：#3a7a3a → 渐变(#3e7a4e → #3a7a3a)
- 练习计数文字颜色层次分明：
  * 低强度：#a5e6d0
  * 中强度：#b5f0dd
  * 高强度：#c5ffea（更明亮）
...
#### 6. 控制区域样式
- 新增 `.calendar-controls` 样式
- 圆角6px，与日历容器协调
- 复选框间距20px，布局清晰

**代码变更：**
- `style.css`: 
  - 优化 `.calendar-container` 及所有子元素样式
  - 添加线性渐变背景
  - 增强字体粗细和间距
  - 改进hover和today状态效果
  - 新增 `.calendar-controls` 样式
